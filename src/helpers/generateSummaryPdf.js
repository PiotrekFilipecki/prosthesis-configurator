...
function addMeasurements(doc, details, startLine) {
  let line = startLine;
  ...
  Object.keys(details.measurments).forEach((key) => {
    const measurement = details.measurments[key];
    doc.text(
...
function addColors(doc, personalize, startLine) {
  let line = startLine;
  let column = 10;
  ...
  Object.keys(personalize.type[personalize.active_type]).forEach((key, index) => {
    const element = personalize.type[personalize.active_type][key];

    doc.text(
      `${element.name}: ${element.selectedColor} ${element.selectedFinishing}`,
      column,
      line
    );
...
function addOrderInfo(doc, details) {
  doc.addPage('a4', 'p');
  doc.setFontSize(15);
  ...
  Object.keys(details.orderInfo).forEach((key) => {
    const field = details.orderInfo[key];
...
export async function generateSummaryPdf({ targetElement, details, personalize }) {
  if (!targetElement) {
    throw new Error('Summary preview element is missing.');
  }
